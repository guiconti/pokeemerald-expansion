...
void CreateEnemyEventMon(void)
{
    ...
    ZeroEnemyPartyMons();
    CreateEventMon(&gEnemyParty[0], species, level, MAX_PER_STAT_IVS, FALSE, 0, OT_ID_PLAYER_ID, 0);
    if (itemId)
    {
        u8 heldItem[2];
        heldItem[0] = itemId;
...
// Gui code below
// Defining pokemon lists
// All pokemons except some really special ones
...
u16 PickTierThreePokemon(const u8 *types, u8 lengthOfTypes, u8 seeded, u16 seed) {
    ...
    if (type == TYPE_NORMAL) {
        return GeneratePokemonFromList(tierThreeNormalSpecies, TIER_THREE_NORMAL_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FIGHTING) {
        return GeneratePokemonFromList(tierThreeFightingSpecies, TIER_THREE_FIGHTING_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FLYING) {
        return GeneratePokemonFromList(tierThreeFlyingSpecies, TIER_THREE_FLYING_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_POISON) {
        return GeneratePokemonFromList(tierThreePoisonSpecies, TIER_THREE_POISON_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GROUND) {
        return GeneratePokemonFromList(tierThreeGroundSpecies, TIER_THREE_GROUND_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ROCK) {
        return GeneratePokemonFromList(tierThreeRockSpecies, TIER_THREE_ROCK_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_BUG) {
        return GeneratePokemonFromList(tierThreeBugSpecies, TIER_THREE_BUG_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GHOST) {
        return GeneratePokemonFromList(tierThreeGhostSpecies, TIER_THREE_GHOST_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_STEEL) {
        return GeneratePokemonFromList(tierThreeSteelSpecies, TIER_THREE_STEEL_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FIRE) {
        return GeneratePokemonFromList(tierThreeFireSpecies, TIER_THREE_FIRE_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_WATER) {
        return GeneratePokemonFromList(tierThreeWaterSpecies, TIER_THREE_WATER_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GRASS) {
        return GeneratePokemonFromList(tierThreeGrassSpecies, TIER_THREE_GRASS_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ELECTRIC) {
        return GeneratePokemonFromList(tierThreeElectricSpecies, TIER_THREE_ELECTRIC_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_PSYCHIC) {
        return GeneratePokemonFromList(tierThreePsychicSpecies, TIER_THREE_PSYCHIC_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ICE) {
        return GeneratePokemonFromList(tierThreeIceSpecies, TIER_THREE_ICE_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_DRAGON) {
        return GeneratePokemonFromList(tierThreeDragonSpecies, TIER_THREE_DRAGON_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_DARK) {
        return GeneratePokemonFromList(tierThreeDarkSpecies, TIER_THREE_DARK_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FAIRY) {
        return GeneratePokemonFromList(tierThreeFairySpecies, TIER_THREE_FAIRY_SPECIES_COUNT, seeded, seed);
    }
    return GeneratePokemonFromList(tierThreeSpecies, TIER_THREE_SPECIES_COUNT, seeded, seed);
}
...
u16 PickTierTwoPokemon(const u8 *types, u8 lengthOfTypes, u8 seeded, u16 seed) {
    ...
    if (type == TYPE_NORMAL) {
        return GeneratePokemonFromList(tierTwoNormalSpecies, TIER_TWO_NORMAL_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FIGHTING) {
        return GeneratePokemonFromList(tierTwoFightingSpecies, TIER_TWO_FIGHTING_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FLYING) {
        return GeneratePokemonFromList(tierTwoFlyingSpecies, TIER_TWO_FLYING_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_POISON) {
        return GeneratePokemonFromList(tierTwoPoisonSpecies, TIER_TWO_POISON_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GROUND) {
        return GeneratePokemonFromList(tierTwoGroundSpecies, TIER_TWO_GROUND_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ROCK) {
        return GeneratePokemonFromList(tierTwoRockSpecies, TIER_TWO_ROCK_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_BUG) {
        return GeneratePokemonFromList(tierTwoBugSpecies, TIER_TWO_BUG_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GHOST) {
        return GeneratePokemonFromList(tierTwoGhostSpecies, TIER_TWO_GHOST_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_STEEL) {
        return GeneratePokemonFromList(tierTwoSteelSpecies, TIER_TWO_STEEL_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FIRE) {
        return GeneratePokemonFromList(tierTwoFireSpecies, TIER_TWO_FIRE_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_WATER) {
        return GeneratePokemonFromList(tierTwoWaterSpecies, TIER_TWO_WATER_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GRASS) {
        return GeneratePokemonFromList(tierTwoGrassSpecies, TIER_TWO_GRASS_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ELECTRIC) {
        return GeneratePokemonFromList(tierTwoElectricSpecies, TIER_TWO_ELECTRIC_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_PSYCHIC) {
        return GeneratePokemonFromList(tierTwoPsychicSpecies, TIER_TWO_PSYCHIC_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ICE) {
        return GeneratePokemonFromList(tierTwoIceSpecies, TIER_TWO_ICE_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_DRAGON) {
        return GeneratePokemonFromList(tierTwoDragonSpecies, TIER_TWO_DRAGON_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_DARK) {
        return GeneratePokemonFromList(tierTwoDarkSpecies, TIER_TWO_DARK_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FAIRY) {
        return GeneratePokemonFromList(tierTwoFairySpecies, TIER_TWO_FAIRY_SPECIES_COUNT, seeded, seed);
    }
    return GeneratePokemonFromList(tierTwoSpecies, TIER_TWO_SPECIES_COUNT, seeded, seed);
}
...
u16 PickTierOnePokemon(const u8 *types, u8 lengthOfTypes, bool8 includeLegendary, u8 seeded, u16 seed) {
    ...
    if (type == TYPE_NORMAL) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneNormalSpecies, TIER_ONE_NORMAL_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesNormalSpecies, TIER_ONE_WITHOUT_LEGENDARIES_NORMAL_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FIGHTING) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneFightingSpecies, TIER_ONE_FIGHTING_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesFightingSpecies, TIER_ONE_WITHOUT_LEGENDARIES_FIGHTING_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FLYING) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneFlyingSpecies, TIER_ONE_FLYING_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesFlyingSpecies, TIER_ONE_WITHOUT_LEGENDARIES_FLYING_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_POISON) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOnePoisonSpecies, TIER_ONE_POISON_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesPoisonSpecies, TIER_ONE_WITHOUT_LEGENDARIES_POISON_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GROUND) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneGroundSpecies, TIER_ONE_GROUND_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesGroundSpecies, TIER_ONE_WITHOUT_LEGENDARIES_GROUND_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ROCK) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneRockSpecies, TIER_ONE_ROCK_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesRockSpecies, TIER_ONE_WITHOUT_LEGENDARIES_ROCK_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_BUG) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneBugSpecies, TIER_ONE_BUG_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesBugSpecies, TIER_ONE_WITHOUT_LEGENDARIES_BUG_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GHOST) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneGhostSpecies, TIER_ONE_GHOST_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesGhostSpecies, TIER_ONE_WITHOUT_LEGENDARIES_GHOST_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_STEEL) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneSteelSpecies, TIER_ONE_STEEL_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesSteelSpecies, TIER_ONE_WITHOUT_LEGENDARIES_STEEL_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FIRE) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneFireSpecies, TIER_ONE_FIRE_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesFireSpecies, TIER_ONE_WITHOUT_LEGENDARIES_FIRE_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_WATER) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneWaterSpecies, TIER_ONE_WATER_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesWaterSpecies, TIER_ONE_WITHOUT_LEGENDARIES_WATER_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GRASS) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneGrassSpecies, TIER_ONE_GRASS_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesGrassSpecies, TIER_ONE_WITHOUT_LEGENDARIES_GRASS_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ELECTRIC) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneElectricSpecies, TIER_ONE_ELECTRIC_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesElectricSpecies, TIER_ONE_WITHOUT_LEGENDARIES_ELECTRIC_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_PSYCHIC) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOnePsychicSpecies, TIER_ONE_PSYCHIC_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesPsychicSpecies, TIER_ONE_WITHOUT_LEGENDARIES_PSYCHIC_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ICE) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneIceSpecies, TIER_ONE_ICE_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesIceSpecies, TIER_ONE_WITHOUT_LEGENDARIES_ICE_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_DRAGON) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneDragonSpecies, TIER_ONE_DRAGON_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesDragonSpecies, TIER_ONE_WITHOUT_LEGENDARIES_DRAGON_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_DARK) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneDarkSpecies, TIER_ONE_DARK_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesDarkSpecies, TIER_ONE_WITHOUT_LEGENDARIES_DARK_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FAIRY) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(tierOneFairySpecies, TIER_ONE_FAIRY_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(tierOneWithoutLegendariesFairySpecies, TIER_ONE_WITHOUT_LEGENDARIES_FAIRY_SPECIES_COUNT, seeded, seed);
    }
    if (includeLegendary == TRUE) {
        return GeneratePokemonFromList(tierOneSpecies, TIER_ONE_SPECIES_COUNT, seeded, seed);
    }
...
u16 PickFromAllPokemon(const u8 *types, u8 lengthOfTypes, bool8 includeLegendary, u8 seeded, u16 seed) {
    ...
    if (type == TYPE_NORMAL) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allNormalSpecies, ALL_NORMAL_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryNormalSpecies, NON_LEGENDARY_NORMAL_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FIGHTING) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allFightingSpecies, ALL_FIGHTING_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryFightingSpecies, NON_LEGENDARY_FIGHTING_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FLYING) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allFlyingSpecies, ALL_FLYING_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryFlyingSpecies, NON_LEGENDARY_FLYING_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_POISON) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allPoisonSpecies, ALL_POISON_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryPoisonSpecies, NON_LEGENDARY_POISON_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GROUND) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allGroundSpecies, ALL_GROUND_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryGroundSpecies, NON_LEGENDARY_GROUND_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ROCK) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allRockSpecies, ALL_ROCK_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryRockSpecies, NON_LEGENDARY_ROCK_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_BUG) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allBugSpecies, ALL_BUG_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryBugSpecies, NON_LEGENDARY_BUG_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GHOST) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allGhostSpecies, ALL_GHOST_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryGhostSpecies, NON_LEGENDARY_GHOST_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_STEEL) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allSteelSpecies, ALL_STEEL_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendarySteelSpecies, NON_LEGENDARY_STEEL_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FIRE) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allFireSpecies, ALL_FIRE_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryFireSpecies, NON_LEGENDARY_FIRE_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_WATER) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allWaterSpecies, ALL_WATER_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryWaterSpecies, NON_LEGENDARY_WATER_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_GRASS) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allGrassSpecies, ALL_GRASS_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryGrassSpecies, NON_LEGENDARY_GRASS_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ELECTRIC) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allElectricSpecies, ALL_ELECTRIC_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryElectricSpecies, NON_LEGENDARY_ELECTRIC_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_PSYCHIC) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allPsychicSpecies, ALL_PSYCHIC_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryPsychicSpecies, NON_LEGENDARY_PSYCHIC_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_ICE) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allIceSpecies, ALL_ICE_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryIceSpecies, NON_LEGENDARY_ICE_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_DRAGON) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allDragonSpecies, ALL_DRAGON_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryDragonSpecies, NON_LEGENDARY_DRAGON_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_DARK) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allDarkSpecies, ALL_DARK_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryDarkSpecies, NON_LEGENDARY_DARK_SPECIES_COUNT, seeded, seed);
    }
    if (type == TYPE_FAIRY) {
        if (includeLegendary == TRUE) {
            return GeneratePokemonFromList(allFairySpecies, ALL_FAIRY_SPECIES_COUNT, seeded, seed);
        }
        return GeneratePokemonFromList(nonLegendaryFairySpecies, NON_LEGENDARY_FAIRY_SPECIES_COUNT, seeded, seed);
    }
    if (includeLegendary == TRUE) {
        return GeneratePokemonFromList(allSpecies, ALL_SPECIES_COUNT, seeded, seed);
    }
